...
namespace Parallel_XPBD.Collisions
{
    public class EllipsoidMap
    {
        ...
        public void AddEllipsoidsToQueue(Ellipsoid[] ellipsoids)
        {
            if (_ellipsoidQueue.Count == 0) _ellipsoidQueue = ellipsoids.ToList();
            else _ellipsoidQueue.AddRange(ellipsoids);
        }

        public void StartFillHashMap(XpbdMesh toSimulate, float gridSize)
        {
            SaveGlobalEllipsoidInLocalSpace(_ellipsoidQueue.ToArray(), toSimulate, gridSize);
            _ellipsoidQueue.Clear();
        }

        private void SaveGlobalEllipsoidInLocalSpace(Ellipsoid[] ellipsoids, XpbdMesh toSimulate,
            float gridSize)
        {
            _gridSize = gridSize;
            NativeArray<Ellipsoid> inEllipsoids = new NativeArray<Ellipsoid>(ellipsoids, Allocator.TempJob);
            ...
            ToLocalEllipsoidsJob toLocalJob = new ToLocalEllipsoidsJob()
            {
                ...
                InEllipsoids = inEllipsoids,
                OutEllipsoids = outEllipsoids
            };
            JobHandle job = toLocalJob.ScheduleParallel(ellipsoids.Length, 32, default);
            SaveGridPositionsParallel(toLocalJob.OutEllipsoids, gridSize, job);
        }

        public JobHandle AccessHashMapParallel(NativeArray<float3> positions, JobHandle previousJob,
            int currentSubstepIndex, int maxSubsteps)
        {
        ...
        public void SaveGridPositionsParallel(NativeArray<Ellipsoid> ellipsoids, float gridSize, JobHandle previousJob)
        {
            if (_fillChainIsRunning) CompleteHashMapFill();
            _nativeEntryList = new NativeList<EllipsoidHashMapEntry>(50000, Allocator.TempJob);

            _fillJob = new FillEllipsoidHashMap()
            {
                Result = _nativeEntryList.AsParallelWriter(),
                GridSize = gridSize,
                Ellipsoids = ellipsoids
            };

            JobHandle fillHandle = _fillJob.Schedule(ellipsoids.Length, 64, previousJob);

            var sortJob = new SortJob
            {
                HashMap = _nativeEntryList.AsDeferredJobArray()
            };
        ...
        private struct FillEllipsoidHashMap : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                float3 startPos = Ellipsoids[index].Position - Ellipsoids[index].Velocity;
                float3 endPos = Ellipsoids[index].Position;

                float3 worldMinStart = startPos - extent;
                float3 worldMinEnd = endPos - extent;

                float3 worldMaxStart = startPos + extent;
                float3 worldMaxEnd = endPos + extent;

                int3 iMin = math.min((int3)math.floor(worldMinStart / GridSize),
                    (int3)math.floor(worldMinEnd / GridSize));
                int3 iMax = math.max((int3)math.floor(worldMaxStart / GridSize),
                    (int3)math.floor(worldMaxEnd / GridSize));
                ...
                for (int x = iMin.x; x <= iMax.x; x++)
                {
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        float3 ellPos = Ellipsoids[index].Position - ((float)i / 10f) * Ellipsoids[index].Velocity;
                        if (!SphereIntersectsAABB(ellPos, rMax2, cellMin, cellMax))
                            continue;

                        if (EllipsoidIntersectsAABB_Sampled(ellPos, RT, invRadii, cellMin, cellMax))
                        {
            ...
            private static float3 OrientedEllipsoidExtent(quaternion rot, float3 radii)
            {
                ...
                float3 r2 = radii * radii;

                float3 row0 = new float3(R.c0.x, R.c1.x, R.c2.x);
                float3 row1 = new float3(R.c0.y, R.c1.y, R.c2.y);
                float3 row2 = new float3(R.c0.z, R.c1.z, R.c2.z);

                float ex = math.sqrt(math.dot(row0 * row0, r2));
                float ey = math.sqrt(math.dot(row1 * row1, r2));
                float ez = math.sqrt(math.dot(row2 * row2, r2));
    ...
    {
        [DeallocateOnJobCompletion] [ReadOnly] public NativeArray<Ellipsoid> InEllipsoids;
        ...
        public void Execute(int i)
        {
            ...
            OutEllipsoids[i] = new Ellipsoid
            {
                ...
                Rotation = rotLocal,
                HalfAxis = s,
                Velocity = InEllipsoids[i].Velocity,
                Index = InEllipsoids[i].Index
            };
        }
    ...
    public struct AccessHashMap : IJobParallelFor
    {
        ...
        [ReadOnly] public int currentSubstep;
        ...
        public void Execute(int index)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                if(index == HashMap[i].Target.Index)continue;
                float3 ellipsoidPosition = HashMap[i].Target.Position -
                                           (((float)maxSubsteps - (float)currentSubstep) / (float)maxSubsteps) *
                                           HashMap[i].Target.Velocity;

                float distance = SdEllipsoid(AccessPoints[index], ellipsoidPosition, HashMap[i].Target.Rotation,
                    HashMap[i].Target.HalfAxis);
                if (distance > 0f) continue;

                float3 direction = fastestExit_world_fast(AccessPoints[index], ellipsoidPosition,
                    HashMap[i].Target.Rotation, HashMap[i].Target.HalfAxis);

                result += normalize(direction) * -1 * distance;
            }
